    ...
    <div className="mx-auto max-w-5xl pb-20">
      ...
      <section className="grid gap-12 border-b border-border py-12 lg:grid-cols-[220px_1fr]">
        ...
        <div className="space-y-5">
          ...
          <label className="flex items-center justify-between gap-5 border-b border-border pb-5"><span><strong className="block text-sm">{t("Email notifications", "邮件提醒")}</strong><span className="mt-1 block text-sm text-foreground/60">{t("Receive publication updates outside the site.", "在站外接收新资源提醒。")}</span></span><button type="button" onClick={() => setProfile({ ...profile, notificationEmail: !profile.notificationEmail, notificationDigest: profile.notificationEmail ? "off" : (profile.notificationDigest === "off" ? "weekly" : profile.notificationDigest) })} className={`relative h-7 w-12 rounded-full transition-colors ${profile.notificationEmail ? "bg-emerald-600" : "bg-muted"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition-transform ${profile.notificationEmail ? "translate-x-6" : "translate-x-1"}`} /></button></label>
